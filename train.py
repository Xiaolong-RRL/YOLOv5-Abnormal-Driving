...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    if RANK in [-1, 0]:
        ...
        if loggers.wandb:  # 是否启用wandb可视化训练
    ...
    pretrained = weights.endswith('.pt')  # 载入模型(权重文件)
    if pretrained:
        with torch_distributed_zero_first(LOCAL_RANK):
        ...
        # 也就是在main函数中 如果使用resume 则opt.cfg, opt.weights, opt.resume = '', ckpt, True  # reinstate
        ...
        # 如果使用resume 就要筛选字典中的键值对(从cfg.yaml文件中加载到ckpt['model']文件)
        # 将exclude(也就是anchors信息)删除 这样ckpt中的anchor信息就不会在下面的load操作中 覆盖掉opt.cfg中的anchors信息了
    ...
    # Freeze 冻结权重层
    # 这里只是给了冻结权重层的一个例子 但是作者并不建议冻结权重层
    # 训练全部层参数 可以得到更好的性能 当然也会更慢
    ...
    # Resume 如果不使用resume的话 下面这些if条件中的语句都不会执行
    ...
    if pretrained:
        ...
        # Epochs 不使用resume start_epoch = -1 + 1 = 0
    ...
    # DP mode DataParallel模式 仅支持单机多卡 参照:https://github.com/ultralytics/yolov5/issues/475
    # rank为进程编号, 如果设置为rank=-1并且有多块gpu 则使用DataParallel模式 效果并不好(分布不平均)
    ...
    # SyncBatchNorm 多卡训练时是否使用同步BN 使用SyncBatchNorm后训练会更耗时
    # 训练带有BN结构的网络时使用SyncBatchNorm采用意义
    ...
    if RANK in [-1, 0]:
        ...
        if not resume:
            labels = np.concatenate(dataset.labels, 0)  # 数据集中所有的target [929, 5]
            ...
            """ Pytorch模型加速和减少显存的trick
            开启半精度 加快运行速度 减少GPU占用 并且只有不明显的accuracy损失
            这一步要放在模型载入GPU之前，即放到model.cuda()之前
    ...
    for epoch in range(start_epoch, epochs):
        ...
        # 如果为True 进行图片采样策略(按数据集各类别权重采样) 默认为False
