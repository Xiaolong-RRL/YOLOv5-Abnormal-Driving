...
def mosaic(imgs, labels, img_size=640):
    ...
    # 测试 (1280,1280,3)->(640,640,3) 的转化
    # new_img4 = cv2.warpAffine(img4, np.eye(3)[:2], dsize=(s, s), borderValue=(114, 114, 114))
    # if img4[:640, :640].all() == new_img4.all():
    #     print(True)  # 结果为True
...
def pix_augment(img, method):
    ...
    # 剪切 大概是变成平行四边形的样子
    # 保持图形上各点的某一坐标值不变 而另一坐标值关于该保持不变坐标值进行线性变换
...
def pix_main():
    ...
    cv2.waitKey(0)
    # 进行数据增强
    out_img = pix_augment(ori_img, method='shear')
...
def img_main():
    ...
    # Mosaic数据增强
    out_img, out_labels = img_augment(ori_imgs, ori_labels, method='mosaic')
    # 绘图
