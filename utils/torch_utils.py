...
def select_device(device='', batch_size=0, newline=True):
    ...
    if cpu:
        ...
        # 如果输入device不为空  device=GPU
        # 直接设置 CUDA environment variable = device 加入CUDA可用设备
    ...
    # 如果cuda可用 且 输入device != cpu 则 cuda=True 反之cuda=False
...
def fuse_conv_and_bn(conv, bn):
    ...
    # 得到融合后的参数w = w_bn * w_conv      torch.mm: 对两个矩阵相乘
...
def model_info(model, verbose=False, img_size=640):
    ...
    try:  # FLOPs
        ...
        # input
        # img = torch.zeros((1, model.yaml.get('ch', 3), stride * 8, stride * 8), device=next(model.parameters()).device)
        img = torch.zeros((1, model.yaml.get('ch', 3), stride, stride), device=next(model.parameters()).device)  # input
        flops = profile(deepcopy(model), inputs=(img,), verbose=False)[0] / 1E9 * 2  # stride GFLOPs
        img_size = img_size if isinstance(img_size, list) else [img_size, img_size]  # expand if int/float
        fs = ', %.1f GFLOPs' % (flops * img_size[0] / stride * img_size[1] / stride)  # 640x640 GFLOPs
        # fs = ', %.1f GFLOPs' % (flops * img_size[0] / (stride * 8) * img_size[1] / (stride * 8))  # 640x640 GFLOPs
